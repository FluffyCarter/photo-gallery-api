...
const LOCAL_DB_CONFIG = {
  ...
  user: 'gallery_app',  // или 'postgres' если не работает
  password: '1812'      // ваш пароль
};
...
function scanDirectoryForImages(dir) {
  ...
  function walk(currentPath) {
    try {
      ...
      console.error(`⚠️  Ошибка сканирования ${currentPath}:`, error.message);
    }
  }
...
async function processUpload(folderPath, clearExisting = false) {
  ...
  try {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      try {
        ...
        if (stats.size > MAX_FILE_SIZE) {
          console.log(`\n⚠️  Пропускаю "${file.filename}" - слишком большой (${(stats.size / 1024 / 1024).toFixed(1)} MB)`);
          skipped++;
          continue;
        }
        
        // Проверяем дубликаты (если не очищали таблицу)
        if (!clearExisting) {
        ...
        // Показываем каждое 10-е или последнее изображение
    ...
    console.log(`⚠️  Пропущено (размер):  ${skipped}`);
    console.log(`❌ Ошибок:              ${errors}`);
    console.log('='.repeat(50));
    
    // Показываем итоговое количество в БД
